	...
	private static DictionaryEntry update_entry(DictionaryEntry e) throws SBOLConversionException, IOException, SBOLValidationException {
		...
		try {
			...
			if(e.uri==null) {
				...
				synbiohubEntry = new DictionaryEntry(e);
			}
			...
			TopLevel topLevelEntity = e.document.getTopLevel(local_uri);
			...
			do {
				Annotation modifiedDateAnnotation = topLevelEntity.getAnnotation(MODIFIED);
			...
			if(e.type.equalsIgnoreCase("CHEBI")) {
				...
				if(chebiURI == null) {
					...
					chebiURI = getCHEBIURI(topLevelEntity);
					if(chebiURI == null) {
						if(synbiohubEntry != null) {
							synbiohubEntry.attributeDefinition = null;
						}
						...
						try {
							...
							setCHEBIURI(topLevelEntity, chebiURI);
							e.changed = true;
				...
				// entity URI
				URI entityChebiURI = getCHEBIURI(topLevelEntity);
				if(synbiohubEntry != null) {
					synbiohubEntry.attributeDefinition = entityChebiURI;
				}
			...
			if(!validateEntityType(topLevelEntity, e.type)) {
				if(chebiTypeIsInRole(topLevelEntity, e.type)) {
			...
			if(e.attribute) {
				...
				boolean entity_is_stub = (topLevelEntity.getAnnotation(STUB_ANNOTATION) != null);
			...
			if(e.name!=null && !e.name.equals(topLevelEntity.getName())) {
				if(synbiohubEntry != null) {
					synbiohubEntry.name = topLevelEntity.getName();
				}

				if(reverseSync) {
					ValueRange update =
							DictionaryAccessor.writeCellData(e, "Common Name",
									topLevelEntity.getName());
					e.spreadsheetUpdates.add(update);
				} else {
					topLevelEntity.setName(e.name);
					e.changed = true;
			...
			for(String labKey : e.labUIDs.keySet()) {
				...
				List<Annotation> annotations = topLevelEntity.getAnnotations();
				...
				if(!labIds.equals(synBioHubIds)) {
					if(synbiohubEntry != null) {
						...
						synbiohubEntry.labUIDs.put(labKey, synBioHubIds);
					}
					...
					if(reverseSync) {
						...
						replaceOldAnnotations(topLevelEntity, labQKey, labIds);
						e.changed = true;
					}
			...
			Set<String> synbiohubAliases = new HashSet<String>();
			QName nameAliasKey = new QName("http://purl.org/dc/terms/", "alternative", "alternative");
			//Go over each alias entities from SBOL
			List<Annotation> annotations = topLevelEntity.getAnnotations();
			for (Annotation ann : annotations) {
				if(ann.getQName().equals(nameAliasKey)) {
					synbiohubAliases.add(ann.getStringValue());
				}
			}
			if(synbiohubEntry != null) {
				synbiohubEntry.aliasNames = synbiohubAliases;
			}
			if(!e.aliasNames.equals(synbiohubAliases)) {
				if(reverseSync) {
					String cellValue = "";
					Object[] sbhAlias_arr = synbiohubAliases.toArray();
					for(int index = 0 ; index< sbhAlias_arr.length; index++) {
						cellValue += sbhAlias_arr[index].toString();
						if(index < (sbhAlias_arr.length-1)) {
							cellValue += "; ";
						}
					}
					ValueRange update =
							DictionaryAccessor.writeCellData(e, "Alias Names", cellValue);
					e.spreadsheetUpdates.add(update);
				} else {
					replaceOldAnnotations(topLevelEntity, nameAliasKey, e.aliasNames);
					e.changed = true;
					e.report.success("Updated Alias Names", true); 
				}
			...
			if(!e.type.equalsIgnoreCase("CHEBI")) {
				...
				Set<URI> derivations = topLevelEntity.getWasDerivedFroms();
				...
				if(e.definitionURIColumn != null) {
					if(synbiohubEntry != null) {
						synbiohubEntry.attributeDefinition = entityDerivation;
					}
					...
					if(e.attributeDefinition == null) {
						...
						if(entityDerivation != null) {
							...
							if(reverseSync) {
								...
								derivations.clear();
								topLevelEntity.setWasDerivedFroms(derivations);
								e.changed = true;
			...
			if(e.changed) {
				e.modifiedDate = now;
				replaceOldAnnotations(topLevelEntity, MODIFIED, e.getModifiedDate());
			}
